...
// ex) scrollViewTopAct('.header_inr', '.header_inr', 'fixed', 50);
...
// ex) scrollViewBtmAct('.footer', '.btn_top', 'hidden', 50);
// ex) 스크롤 할 때 .footer가 화면 하단에서 올라오면 .btn_top에 hidden 클래스 삽입
function scrollViewBtmAct(target1, target2, classes, offset) {
	var $target1 = $(target1),
		$target2 = $(target2);
	if ($target1.length) {
		$target1.each(function() {
...
// --------------------------------
// scroll ani
// ex) scrollAni('.sec06', '.sec06_inr', 'slideInUp delay-1_4s faster infinite', 50);
